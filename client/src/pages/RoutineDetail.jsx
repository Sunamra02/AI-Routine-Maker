...
const RoutineDetail = () => {
	...
	const toggleTask = async (taskId) => {
		...
		setRoutine((previous) => ({
			...previous,
			tasks: previous.tasks.map((item) => item.id === taskId ? { ...item, completed } : item),
		}));
		try {
	...
	const removeRoutine = async () => {
		...
		try {
			...
			showToast(error.message || 'Could not delete routine.', 'error');
		}
	};
	...
	return (
		<div className="flex-1 py-10 px-4 max-w-4xl mx-auto w-full space-y-6">
			<div className="flex flex-wrap justify-between gap-3">
				<div>
					<Link to="/my-routine" className="text-sm text-blue-600 font-semibold">
						← All routines
					</Link>
					<h1 className="text-3xl font-bold text-slate-900 mt-2">{routine.goal}</h1>
					<p className="text-slate-600 text-sm">
						Scheduled for {routine.routineDate || 'an unscheduled legacy date'} · {routine.wakeUpTime}–{routine.sleepTime}
					</p>
				</div>
				<div id="routine-actions" className="self-center">
					<div className="flex gap-2">
						<Link to={`/edit-routine/${id}`} className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-semibold cursor-pointer">
							Edit
						</Link>
						<button onClick={removeRoutine} className="px-4 py-2 rounded-xl border border-red-200 hover:bg-red-200 text-red-700 font-semibold cursor-pointer">
							Delete
						</button>
					</div>
				</div>
